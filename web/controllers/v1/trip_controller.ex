defmodule Mikeworks.V1.TripController do
  use Mikeworks.Web, :controller
  ...
  def create(conn, %{"trip" => trip_params}) do
    ...
    case Repo.insert(changeset) do
      {:ok, trip} ->
        conn
        |> put_status(:created)
        |> put_resp_header("location", v1_trip_path(conn, :show, trip))
        |> render("show.json", trip: trip)
      {:error, changeset} ->
        conn
        |> put_status(:unprocessable_entity)
        |> render(Mikeworks.ChangesetView, "error.json", changeset: changeset)
    end
  end
